...
exports.getFeatureFn = exports.getFeatures = exports.getCustomFeatures = undefined;
...
var _selectors = __webpack_require__(42);
...
var customFeatureDefaults = function customFeatureDefaults(feature) {
  return {
    binder: function binder(wrapper, mod) {
      var bound = wrapper(mod);
      return isObj(bound) ? bound : _defineProperty({}, mod.name, bound);
    },
    combiner: function combiner(mod, deps, Module) {
      return _defineProperty({}, feature, _extends({}, mod, deps));
    },
    fallback: function fallback(mod) {
      return {};
    },
    wrapper: function wrapper(featureFn) {
      return function (mod) {
        return typeof featureFn === "function" ? featureFn(mod) : featureFn;
      };
    }
  };
};

var getCustomFeatures = function getCustomFeatures() {
  var features = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
  return features.reduce(function (acc, _ref3) {
    var feature = _ref3.feature,
        rest = _objectWithoutProperties(_ref3, ["feature"]);

    return _extends({}, acc, _defineProperty({}, feature, _extends({}, customFeatureDefaults(feature), rest)));
  }, {});
};
...
var getFeatureFn = function getFeatureFn(features) {
...
exports.getCustomFeatures = getCustomFeatures;
exports.getFeatures = getFeatures;
exports.getFeatureFn = getFeatureFn;
...
exports.contextWrapper = exports.providerWrapper = undefined;
...
var ModuleProvider = function (_Component) {
  _inherits(ModuleProvider, _Component);

  function ModuleProvider() {
    _classCallCheck(this, ModuleProvider);

    return _possibleConstructorReturn(this, (ModuleProvider.__proto__ || Object.getPrototypeOf(ModuleProvider)).apply(this, arguments));
  }
...
var providerWrapper = function providerWrapper(View, Module) {
...
var contextWrapper = function contextWrapper(Component) {
  var ModularizedComponent = function ModularizedComponent(props, context) {
    return _react2.default.createElement(Component, _extends({}, props, { Module: context.module }));
  };
  ModularizedComponent.contextTypes = { module: _propTypes2.default.object };
...
var _bind = __webpack_require__(54);

var _bind2 = _interopRequireDefault(_bind);

var _combine = __webpack_require__(55);
...
var unpack = function unpack(features, featureOrder) {
  var modFeatures = {};
  var depFeatures = {};
  // Sorts the keys (since the binders depend on each-other)
  var orderedKeys = Object.keys(features).sort(function (curr, next) {
    return featureOrder.indexOf(curr) - featureOrder.indexOf(next);
  });

  orderedKeys.forEach(function (ft) {
    var _features$ft = features[ft],
        module = _features$ft.module,
        dep = _objectWithoutProperties(_features$ft, ["module"]);

    modFeatures[ft] = module;
    depFeatures[ft] = dep;
  });

  return { modFeatures: modFeatures, depFeatures: depFeatures, orderedKeys: orderedKeys };
};

var createModule = function createModule(_ref) {
  var custom = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];

  var _ref$name = _ref.name,
      name = _ref$name === undefined ? "module" : _ref$name,
      features = _objectWithoutProperties(_ref, ["name"]);

  var libFeatures = (0, _features.getFeatures)();
  var customFeatures = (0, _features.getCustomFeatures)(custom);
  var allFeatures = _extends({}, libFeatures, customFeatures);

  var libKeys = Object.keys(libFeatures).filter(function (f) {
    return f !== "views";
  });
  var customKeys = Object.keys(customFeatures);
  var keysOrder = (0, _utils.uniq)([].concat(_toConsumableArray(libKeys), _toConsumableArray(customKeys), ["views"]));

  var bind = (0, _bind2.default)(allFeatures);
  var combine = (0, _combine2.default)(allFeatures);
  var wrappers = (0, _features.getFeatureFn)(customFeatures, "wrapper"); // Import wrappers to use only on custom features

  var _unpack = unpack(features, keysOrder),
      modFeatures = _unpack.modFeatures,
      depFeatures = _unpack.depFeatures,
      orderedKeys = _unpack.orderedKeys;

  var Module = orderedKeys.reduce(function (Module, ft) {
    var modFeature = !!wrappers[ft] ? wrappers[ft](modFeatures[ft]) : modFeatures[ft]; // Wrap if custom
    var depFeature = depFeatures[ft];
...
var Modular = createModule;
var internalFeatures = (0, _features.getFeatures)();
var internalWrappers = (0, _features.getFeatureFn)(internalFeatures, "wrapper");
...
var _binder = __webpack_require__(20);

var _binder2 = _interopRequireDefault(_binder);

var _combiner = __webpack_require__(21);

var _combiner2 = _interopRequireDefault(_combiner);

var _wrapper = __webpack_require__(22);

var _wrapper2 = _interopRequireDefault(_wrapper);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var actions = { binder: _binder2.default, combiner: _combiner2.default, fallback: _binder.fallback, wrapper: _wrapper2.default };
...
var binder = function binder(wrapper, mod) {
  return wrapper(mod.instance);
};
var fallback = function fallback(mod) {};
...
var combiner = function combiner(mod, deps, Module) {
  return { actions: _extends({}, mod, deps) };
};
...
var actionTypes = function actionTypes(types) {
  var instance = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "";
  var separator = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "/";
  return types.reduce(function (acc, type) {
    return _extends({}, acc, _defineProperty({}, type, instance + separator + type));
  }, {});
};

var wrapper = function wrapper(types, actions, config) {
  return function (moduleInstance) {
    var moduleTypes = actionTypes(types, moduleInstance);
    return _extends({}, actions(moduleTypes), { types: moduleTypes });
  };
};
...
var _combiner = __webpack_require__(25);

var _combiner2 = _interopRequireDefault(_combiner);

var _wrapper = __webpack_require__(41);

var _wrapper2 = _interopRequireDefault(_wrapper);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var reducers = { binder: _binder2.default, combiner: _combiner2.default, fallback: _binder.fallback, wrapper: _wrapper2.default };
...
var combiner = function combiner(mod, deps, Module) {
  return { reducer: (0, _redux.combineReducers)(_extends({}, mod, deps)) };
};
...
/* WEBPACK VAR INJECTION */}.call(__webpack_exports__, __webpack_require__(10), __webpack_require__(36)(module)))
...
var wrapper = function wrapper(reducer) {
  return function (moduleInstance, moduleActions) {
    return function () {
      for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
        args[_key] = arguments[_key];
      }

      return reducer.apply(undefined, args.concat([{ instance: moduleInstance, actions: moduleActions }]));
    };
  };
};
...
var _binder = __webpack_require__(43);

var _binder2 = _interopRequireDefault(_binder);

var _combiner = __webpack_require__(44);
...
var selectors = { binder: _binder2.default, combiner: _combiner2.default, fallback: _binder.fallback, wrapper: _wrapper2.default };
...
var binder = function binder(wrapper, mod) {
  return _extends({}, wrapper(mod.instance));
};
var fallback = function fallback(mod) {};
...
var combiner = function combiner(mod, deps, Module) {
  return { selectors: _extends({}, mod, deps) };
};
...
var deepFindModule = function deepFindModule(obj, instance) {
  ...
  for (var key in obj) {
    var val = obj[key];
    if ((typeof val === "undefined" ? "undefined" : _typeof(val)) === "object") {
      var found = deepFindModule(val, instance);
      if (found) {
        return found;
      }
    }
...
var wrapper = function wrapper(selectors) {
  return function (moduleInstance) {
    return selectors(function (state) {
      return deepFindModule(state, moduleInstance);
    });
  };
};
...
var _binder = __webpack_require__(47);

var _binder2 = _interopRequireDefault(_binder);

var _combiner = __webpack_require__(50);
...
var views = { binder: _binder2.default, combiner: _combiner2.default, fallback: _binder.fallback, wrapper: _wrapper2.default };
...
var fallback = function fallback(Module) {
  return function () {
    return _react2.default.createElement(
      "div",
      null,
...
var combiner = function combiner(mod, deps, Module) {
  var view = (0, _wrapper.providerWrapper)(mod, _extends({}, Module, { view: deps }));
...
var getBindFn = function getBindFn() {
  var features = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  ...
  var binders = (0, _features.getFeatureFn)(features, "binder");
  ...
  var fallbacks = (0, _features.getFeatureFn)(features, "fallback");
  ...
  return function (component, wrapper, mod) {
    var binder = binders[component];
    var fallback = fallbacks[component];
...
var getCombineFn = function getCombineFn() {
  var features = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  ...
  var combiners = (0, _features.getFeatureFn)(features, "combiner");

  return function (component, mod, deps, Module) {
    var combiner = combiners[component];
    return combiner ? combiner(mod, deps, Module) : mod;
  };
};

exports.default = getCombineFn;
